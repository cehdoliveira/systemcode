...
{
  return substr(md5(uniqid((string)time(), true)), 0, $size);
}
...
function basic_redir($url, $code = 301, $use_html = false)
{
  ...
  if ($use_html) {
    ...
    $out = ob_get_contents();
    ob_end_clean();
    echo str_replace("{location}", $url, $out);
  } else {
...
{
  $server_name = filter_var(getenv("SERVER_NAME"), FILTER_SANITIZE_SPECIAL_CHARS);
  $server_protocol = filter_var(getenv("SERVER_PROTOCOL"), FILTER_SANITIZE_SPECIAL_CHARS);
  $server_port = filter_var(getenv("SERVER_PORT"), FILTER_SANITIZE_SPECIAL_CHARS);

  if (strtoupper(getenv("HTTPS")) == "ON" || ($https === true)) {
  ...
  $request_server = $server_protocol . "://" . ($auth ? $auth . "@" : "") . $server_name;
...
{
  return filter_var($mail, FILTER_VALIDATE_EMAIL) && (checkdnsrr(preg_replace("/[^@\s]*@(.+)$/", "$1", $mail), 'A') || checkdnsrr(preg_replace("/[^@\s]*@(.+)$/", "$1", $mail), 'MX'));
}
...
$remote_address = filter_var(getenv("REMOTE_ADDR"), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
$http_user_agent = filter_var(getenv("HTTP_USER_AGENT"), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
$referrer = filter_var(getenv("HTTP_REFERER"), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
$request_uri = filter_var(getenv("SCRIPT_NAME"), FILTER_SANITIZE_FULL_SPECIAL_CHARS);
...
function html_notification_print()
{
  if (isset($_SESSION["messages_app"])) {
    foreach ($_SESSION["messages_app"] as $type => $context) {
      echo '<div class="text-center alert alert-' . $type . '" role="alert">';
      echo implode("<br>", $context);
      echo '</div>';
    }
